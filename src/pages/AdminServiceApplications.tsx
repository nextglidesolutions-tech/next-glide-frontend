...
export default function AdminServiceApplications() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const [servicesRes, inquiriesRes] = await Promise.all([
                ...
                fetch(`${apiUrl}/api/services/inquiries/all`)
            ]);

            if (servicesRes.ok && inquiriesRes.ok) {
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5001';
            const response = await fetch(`${apiUrl}/api/services/inquiries/${id}`, {
                method: 'DELETE'
            });
    ...
    const handleResendEmail = async (id: string) => {
        ...
        try {
            const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5001';
            const response = await fetch(`${apiUrl}/api/services/inquiries/${id}/resend`, {
                method: 'POST'
            });
    ...
    const exportToCSV = () => {
        ...
        const dataToExport = selectedServiceId === 'all'
            ? inquiries
        ...
        // Prepare Headers - Dynamic Custom Fields handling is tricky in CSV, we will flatten them or put in one column
        const headers = ['Date', 'Service', 'Full Name', 'Email', 'Phone', 'Company', 'Budget', 'Requirements', 'Custom Responses'];
    ...
    const filteredInquiries = selectedServiceId === 'all'
        ? inquiries
        : inquiries.filter(i => i.serviceId === selectedServiceId || i.serviceName === services.find(s => s._id === selectedServiceId)?.name);
        ...
        <AdminLayout>
            <div className="p-8 h-full flex flex-col">
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <h1 className="text-3xl font-bold text-foreground">Service Applications</h1>
                        <p className="text-muted-foreground">Manage incoming inquiries and applications.</p>
                    </div>
                </div>
                ...
                <div className="bg-card p-4 rounded-xl border shadow-sm mb-6 flex flex-wrap gap-4 items-end justify-between">
                    <div className="w-full md:w-1/3">
                        <Label className="mb-2 block">Filter by Service</Label>
                        <select
                        ...
                        <div className="space-y-6 py-4">
                            <div className="grid grid-cols-2 gap-4 p-4 bg-muted/30 rounded-lg border">
                                <div>
                                    ...
                                    <div className="font-bold text-lg text-blue-700">{selectedInquiry.serviceName}</div>
                                </div>
                                <div>
