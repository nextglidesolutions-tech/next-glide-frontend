...
export default function AdminServices() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const url = view === 'create' ? `${apiUrl}/api/services` : `${apiUrl}/api/services/${formData._id}`;
            ...
            if (response.ok) {
                toast({ title: 'Success', description: `Service ${view === 'create' ? 'created' : 'updated'} successfully` });
                fetchServices();
                setView('list');
                setFormData({});
            } else {
    ...
    const handleEditClick = async (svc: any) => {
        setLoading(true);
        try {
            const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5001';
            const response = await fetch(`${apiUrl}/api/services/${svc.slug}`);
            if (response.ok) {
                const data = await response.json();
                setFormData(data);
                setView('edit');
            } else {
                toast({ variant: 'destructive', title: 'Error', description: 'Failed to fetch service details' });
            }
        } catch (error) {
                ...
                <div className="p-8 space-y-8 bg-muted/10 min-h-screen">
                    <div className="flex justify-between items-center">
                        <div>
                            ...
                            <p className="text-muted-foreground">Manage your service offerings ({services.length})</p>
                        </div>
                            ...
                            <TableBody>
                                {services.map((svc) => (
                                    <TableRow key={svc._id}>
                                        ...
                                        <TableCell>{svc.startingPrice}</TableCell>
                                        <TableCell className="text-right">
                                            <Button variant="ghost" size="icon" onClick={() => handleEditClick(svc)}>
                                ...
                                {services.length === 0 && <TableRow><TableCell colSpan={4} className="text-center h-24">No services found.</TableCell></TableRow>}
                            </TableBody>
                        </Table>
                    </div>
                </div>
                ...
                <div className="w-[70%] flex flex-col border-r border-border bg-white h-full overflow-y-auto">
                    <div className="p-6 border-b border-border sticky top-0 bg-white z-10 flex justify-between items-center">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-xl font-bold">{view === 'create' ? 'Create Service' : 'Edit Service'}</h2>
                        </div>
                        ...
                        <div className="space-y-6 max-w-2xl">
                            {currentSection === 'listing' && (
                                <div className="animate-in fade-in slide-in-from-left-4 duration-300">
                                    ...
                                    {renderField('Service Name *', 'name')}
                                    {renderField('URL Slug (e.g., advisory-service) *', 'slug')}
                                    {renderField('Short Description *', 'shortDescription', 'textarea')}
                                    <div className="grid grid-cols-2 gap-4">
                                        {renderField('Category', 'category')}
                                        {renderField('Starting Price', 'startingPrice')}
                                    </div>
                                    {renderField('Call To Action Text', 'ctaText', 'text', 'Apply Now')}
                                </div>
                            )}
                ...
                <div className="w-[30%] bg-slate-50 h-full p-8 overflow-y-auto border-l border-border">
                    ...
                    <div className="mb-12">
                        ...
                        <div className="bg-white p-6 rounded-xl border shadow-sm max-w-sm">
                            ...
                            <h3 className="font-bold text-lg mb-2">{formData.name || 'Service Name'}</h3>
                            ...
                            <Button className="w-full" variant="outline">
                                {formData.ctaText || 'Apply Now'} <ArrowRight className="w-4 h-4 ml-2" />
                            </Button>
                        </div>
                    </div>
