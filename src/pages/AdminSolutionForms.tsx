...
export default function AdminSolutionForms() {
    ...
    const fetchSolutions = async () => {
        ...
        try {
            ...
            toast({ variant: 'destructive', title: 'Error', description: 'Failed to load solutions' });
        } finally {
            setLoading(false);
        }
    };

    const handleSolutionSelect = async (slug: string) => {
        setLoading(true);
        try {
            const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5001';
    ...
    const handleSaveForm = async () => {
        ...
        try {
            const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5001';
            const response = await fetch(`${apiUrl}/api/solutions/${selectedSolution._id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    inquiryFormFields: selectedSolution.inquiryFormFields
                }),
            });
            ...
            if (response.ok) {
                ...
                throw new Error('Failed to save solution form');
            }
        } catch (error) {
    ...
    const handleSaveField = () => {
        ...
        const updatedFields = [...(selectedSolution.inquiryFormFields || [])];
        ...
        setSelectedSolution({ ...selectedSolution, inquiryFormFields: updatedFields });
            ...
            <div className="p-8 h-full flex flex-col">
                <div className="flex justify-between items-center mb-8">
                    <div>
                        ...
                        <p className="text-muted-foreground">Customize the enquiry form questions for each solution.</p>
                    </div>
                </div>
                ...
                {!isEditMode ? (
                    <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
                        <Table>
                            ...
                            <TableBody>
                                {solutions.map((sol) => (
                                    <TableRow key={sol._id} className="hover:bg-muted/50 cursor-pointer" onClick={() => handleSolutionSelect(sol.slug)}>
                                        <TableCell className="font-semibold">{sol.name}</TableCell>
                                        <TableCell>{sol.category}</TableCell>
                                        <TableCell>
                                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${sol.inquiryFormFields?.length > 0 ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'
                                                }`}>
                                                {sol.inquiryFormFields?.length || 0} Fields
                                            </span>
                                        </TableCell>
                    ...
                    <div className="flex gap-8 h-full overflow-hidden">
                        ...
                        <div className="w-1/2 flex flex-col bg-white rounded-xl border shadow-sm h-full">
                            <div className="p-4 border-b flex justify-between items-center bg-gray-50 rounded-t-xl">
                                <div>
                                    <h3 className="font-bold text-lg">{selectedSolution.name} Form</h3>
                                    <p className="text-xs text-muted-foreground">Add questions users must answer to apply.</p>
                                </div>
                                <div className="flex gap-2">
                                    <Button variant="outline" size="sm" onClick={() => { setIsEditMode(false); setSelectedSolution(null); }}>
                                        Cancel
                                    </Button>
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-4">
                                {(selectedSolution.inquiryFormFields || []).length === 0 && (
                                ...
                                {(selectedSolution.inquiryFormFields || []).map((field: any, idx: number) => (
                                        ...
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <div className="flex gap-1 opaciy-0 group-hover:opacity-100 transition-opacity">
                                                <Button size="icon" variant="ghost" className="h-8 w-8 text-gray-400 hover:text-blue-600" onClick={() => {
                                                    const updated = [...selectedSolution.inquiryFormFields];
                                                    updated[idx].isVisible = field.isVisible === false ? true : false;
                                                    setSelectedSolution({ ...selectedSolution, inquiryFormFields: updated });
                                                }}>
                        ...
                        <div className="w-1/2 bg-gray-100 rounded-xl p-8 border overflow-y-auto">
                            ...
                            <div className="bg-white rounded-lg shadow-sm border p-6 max-w-md mx-auto">
                                <h2 className="text-xl font-bold mb-1">Apply for {selectedSolution.name}</h2>
                                ...
                                <div className="space-y-4 pt-4 border-t border-dashed">
                                    ...
                                    {(selectedSolution.inquiryFormFields || []).length === 0 && <p className="text-sm italic text-muted-foreground">No extra questions.</p>}
